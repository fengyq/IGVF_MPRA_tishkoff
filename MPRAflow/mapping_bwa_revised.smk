rule assignment_mapping_bwa_ref:
    ...
    shell:
        ...
        samtools dict {input.ref} -o {output.d} &>> {log}
        """
...
rule assignment_mapping_bwa_getBCs:
    ...
    shell:
        ...
        awk -v "OFS=\\t" '{{
            ...
            cigar_ok = ($6 ~ /^[0-9]+M$/);
            if (a[1] !~ /N/) {{
                if (nm_ok && cigar_ok && ($5 >= {params.mapping_quality_min}) && ($4 >= {params.alignment_start_min}) && ($4 <= {params.alignment_start_max}) && (length($10) >= {params.sequence_length_min}) && (length($10) <= {params.sequence_length_max})) {{
